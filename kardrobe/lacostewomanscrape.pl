...
open (my $OUTFILE, '+>:utf8', 'lacoste/womanoutfile.txt');

my $directory = 'lacoste/woman';
...
while (my $file = readdir(DIR)) {
	...
	if ( $name ) {
	    ...
	    print $OUTFILE "#title=".$name."\n"; 
            print $OUTFILE "b1".lc($name)."\n";
            print $OUTFILE "!".lc($name)."\n";
	} else {
	    next;
	}
	...
	print $OUTFILE "#brand=shop.lacoste.com\n";
